  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        <span className="text-xs font-syne font-bold text-zinc-300 uppercase tracking-wider hidden sm:inline">{current.code}</span>
        <motion.svg animate={{ rotate: open ? 180 : 0 }} transition={{ duration: 0.2 }} width="10" height="10" viewBox="0 0 10 10" className="text-zinc-500">
              ...
              return (
                <motion.button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 transition-all cursor-pointer ${
                    isActive ? "bg-violet/20 text-violet" : "text-zinc-200 hover:bg-white/10 hover:text-white"
                  }`}
                >
                  <img src={`/flags/${l.code}.svg`} alt={l.name} className="w-5 h-5 rounded-full object-cover" />
                  <div className="flex flex-col items-start">
                    <span className="text-sm font-syne font-bold">{l.nativeName}</span>
                    <span className="text-[10px] text-zinc-500 uppercase tracking-wider">{l.name}</span>
                  </div>
                  {isActive && <motion.div layoutId="lang-active" className="ml-auto w-2 h-2 rounded-full bg-violet" />}
                </motion.button>
              );
            })}
          </motion.div>
