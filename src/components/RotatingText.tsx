...
const RotatingText = forwardRef<{ next: () => void; previous: () => void; jumpTo: (index: number) => void; reset: () => void }, RotatingTextProps>((props, ref) => {
  ...
  return (
    <motion.span className={cn('text-rotate inline-flex items-center', mainClassName)} {...rest} layout transition={transition as any}>
      <span className="sr-only">{texts[currentTextIndex]}</span>
      <AnimatePresence mode={animatePresenceMode} initial={animatePresenceInitial}>
        <motion.span
          key={currentTextIndex}
          className={cn(splitBy === 'lines' ? 'text-rotate-lines' : 'text-rotate', 'inline-flex', splitBy === 'words' ? 'flex-row-reverse' : '')}
          layout
          aria-hidden="true"
        >
          {elements.map((wordObj, wordIndex, array) => {
            ...
            return (
              <span key={wordIndex} className={cn('text-rotate-word inline-flex', splitLevelClassName)}>
                {splitBy === 'words' ? (
                  <motion.span
                    initial={initial}
                    animate={animate}
                    ...
                    className={cn('text-rotate-element inline-block whitespace-nowrap', elementLevelClassName)}
                  >
